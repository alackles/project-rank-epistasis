...
outfile <- "wilcoxon_addmult.csv"

# columns that represent factors to be sorted by, not numbers
fac_cols <- c("pos_REF", "pos_MUT", "rep", "allele_REF", "allele_MUT")
...
df_ref <- df %>%
  group_by(rep, allele_REF, allele_MUT) %>%
...
rank_epistasis <- function(dframe, mut) {
  df_wilcox <- dframe %>%
    ...
    select(pos_MUT, rep, allele_REF, allele_MUT, W) %>%
    {.}
